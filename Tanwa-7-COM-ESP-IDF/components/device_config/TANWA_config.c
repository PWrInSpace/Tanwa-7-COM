...
esp_err_t TANWA_utility_init() {
    ...
    TANWA_utility.servo_valve[0].config.close_angle = VALVE1_CLOSE_POSITION;
    TANWA_utility.servo_valve[0].config.open_angle = VALVE1_OPEN_POSITION;

    TANWA_utility.servo_valve[1].config.close_angle = VALVE2_CLOSE_POSITION;
    TANWA_utility.servo_valve[1].config.open_angle = VALVE2_OPEN_POSITION;

    TANWA_utility.servo_valve[0].servo.close_angle = VALVE1_CLOSE_POSITION;
    TANWA_utility.servo_valve[0].servo.open_angle = VALVE1_OPEN_POSITION;

    TANWA_utility.servo_valve[1].servo.close_angle = VALVE2_CLOSE_POSITION;
    TANWA_utility.servo_valve[1].servo.open_angle = VALVE2_OPEN_POSITION;
...
esp_err_t TANWA_get_vbat(float* vbat){
    ...
    *vbat = voltage * 12.0331491709f; // (10k + 50k) / 10k (voltage divider)
    return ESP_OK;
}