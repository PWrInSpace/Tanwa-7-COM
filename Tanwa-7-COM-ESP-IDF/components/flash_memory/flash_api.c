...
FlashResult FLASH_init(uint8_t max_files) {
  ...
  if (err != ESP_OK) {
    if (err == ESP_FAIL) {
      ESP_LOGE(
          TAG,
          "Failed to mount or format, use FLASH_format(), to format partition");
  ...
  uint32_t used_size;
  err = esp_spiffs_info(fl.conf.partition_label, &fl.total_size,
                        (size_t*)&used_size);
...
size_t FLASH_get_used_size(void) {
  ...
  ESP_LOGI(TAG, "Used size %d, total size %d", (int)used_size,
           (int)fl.total_size);
  return used_size;
}
